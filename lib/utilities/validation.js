...
const { MAX_DESCRIPTION_LENGTH } = require("../constants/event");
const {
  DAYS,
  MONTHS,
  LAST_100_YEARS,
...
} = require("../constants/other");

// Utilities
const { getTimes } = require("./dateTime");
...
const validatePasswordReset = (form) => {
  const { password } = form;
  const errors = {};
...
const validateCreateEvent = (form) => {
  const {
    name,
    description,
    game,
...
const validateDeleteAccount = (form) => {
